...
const MovieForm = ({ onSubmit, initialValues, submitLabel = 'Add', onCancel }: MovieFormProps) => {
    const {
        ...
        defaultValues: initialValues ?? { title: '', year: 2000, genre: '', image: '', description: '' },
    });

    useEffect(() => {
        reset(initialValues ?? { title: '', year: 2000, genre: '', image: '', description: '' });
    }, [initialValues, reset]);
    ...
    return (
        <form onSubmit={handleSubmit(handleFormSubmit)} className="bg-gray-800 p-6 rounded-lg border border-gray-700 space-y-4">
            ...
            <div>
                <input {...register('image')} placeholder="Image URL (e.g., https://image.tmdb.org/t/p/w500/9gk7adHYeDvHkCSEqAvQNLV5Uge.jpg)" className="w-full p-2 bg-gray-700 text-white rounded border border-gray-600" />
                {errors.image && <p className="text-red-500 text-xs">{errors.image.message}</p>}
            </div>

            <div>
                <textarea {...register('description')} placeholder="Description (optional)" rows={3} className="w-full p-2 bg-gray-700 text-white rounded border border-gray-600" />
